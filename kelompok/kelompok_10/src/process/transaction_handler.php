...
function createTransaction($conn) {
    ...
    // Generate ID transaksi (format: TRX-YYYYMMDD-XXXXX)
    $date = date('Ymd');
    $randNum = str_pad(rand(1, 99999), 5, '0', STR_PAD_LEFT);
    $transaction_id = "TRX-$date-$randNum";

    // Cek apakah ID sudah ada (sangat kecil kemungkinannya)
...
function takeTransaction($conn) {
    ...
    // Cek apakah laundry sudah selesai
...
function processPayment($conn) {
    ...
    // Validasi jumlah bayar minimal sama dengan total harga
...
function deleteTransaction($conn) {
    // Validasi role (kasir atau admin)
    ...
    // Hanya bisa hapus transaksi yang masih Pending dan belum dibayar
    ...
    // Hapus transaksi (akan otomatis hapus status_logs dan payment_history karena ON DELETE CASCADE)
